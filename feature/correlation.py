...
if __name__ == "__main__":
    ...
    pos_df = pos_df.drop(['text', 'sentiment'], axis=1)
    pos_df = pos_df.replace((np.inf, -np.inf, np.nan), 0).reset_index(drop=True)
    pos_arr = pos_df.to_numpy()
    neg_df = df.loc[df['sentiment'] == 0]
    neg_df = neg_df.drop(['text', 'sentiment'], axis=1)
    neg_df = neg_df.replace((np.inf, -np.inf, np.nan), 0).reset_index(drop=True)
